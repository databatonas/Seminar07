...
try /* Блок обработки исключений.*/
{
    ...
    int m = int.Parse(Console.ReadLine()!); /* (m- перевод- строки).Функция Parse () разрешает 
переводить строку, которая состоит из цифр, в целое число. (!)- для обработки исключений.*/
    ...
    int n = int.Parse(Console.ReadLine()!); // n- перевод- столбцы.
    ...
    int[,] matrix = new int[m, n]; /* Здесь int- тип данных, [,]- указатель того, что будет две 
размерности (двумерный массив). Одна размерность- это строки. Вторая размерность- столбцы. 
matrix- наименование массива.
new int- новый массив int. [m,n]- (m- количество строк), (n- количество  столбцов).*/
        ...
        увеличиваем его на единицу и ...*/
        {
            ...
            увеличиваем его на единицу и ...*/
            {
                matrix[i, j] = i + j; /* присваиваем текущему элементу массива сумму текущих значений 
                                         счётчиков строк и столбцов (по условию).*/
            }
        }
    }
