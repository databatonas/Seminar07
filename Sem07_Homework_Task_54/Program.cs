...
try /* Блок обработки исключений.*/
{
    ...
    void SelectionSort(int[,] array)/* В качестве аргумента будет приходить некоторый массив array.*/
    {
        ...
        массива и тогда получится, что j+1 даст общее количество элементов в строке.*/
        {
            ...
            написать количество строк так- matrix.GetLength(0), где (0)- означает строки. */
            {
                int max = array[i, j]; /* Определяем позицию  максимального элемента (max) и 
            присвоим, ей, значение (i,J- изначально она равна нулевому столбцу и нулевой строке) для 
            которого, в дальнейшем, будем производить следующие действия.*/

                for (int k = j + 1; k < matrix.GetLength(1); k++) /* Этот цикл будет перебирать не 
            сортированную часть одной строки, начиная с позиции (j + 1).*/
                {
                    /* Если, при переборке строки, появился элемент, который больше (max), 
                       то эти элементы меняем местами.*/
                    if (array[i, k] > max) (max, array[i, k]) = (array[i, k], max);
                }

                // Замена элементов от большего к меньшему.
                int temp = array[i, j]; /* Введём временную переменную (temp) и присвоим ей значение 
                текущего элемента (после каждого цикла операция производится со следующей строкой, 
                т.е. перескакивает на (i+1)), а...*/
                array[i, j] = max; /* текущему элементу присвоим значение максимального элемента, 
                (т.е. ставим его в начало строки) и повторяем операцию с новой строкой и...*/
                max = temp; /* присваиваем ему значение времнной переменной.*/
            }
    ...
    SelectionSort(matrix); /* Вызов метода, который упорядочит по убыванию элементы каждой строки 
                              двумерного массива.*/
    PrintArray(matrix); /* Вызов метода вывода на печать нашего массива, заполненного случайными числами.*/
}
