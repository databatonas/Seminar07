...
try /* Блок обработки исключений.*/
{
    ...
    {
        for (int i = 0; i < matrix.GetLength(0); i++) /* Далее перебираем строки. Для общего случая 
        можно написать количество строк так- matrix.GetLength(0), где (0)- означает строки. */
        {
            for (int j = 0; j < matrix.GetLength(1); j++) /* (1)- означает столбцы. Здесь обращаемся к 
            конкретному элементу на позиции (i и j) и пишем, через использование генератора псевдослучайных 
            чисел Random(). Next(beginning,end) (возьмём из интервала от beginning до end).*/

                if (i % 2 == 0 && j % 2 == 0) /* Если оба индекса элемента чётные (остаток от деления 
                                                 на два равен нулю), то...*/
                {
                    matrix[i, j] = Convert.ToInt32(Math.Pow((matrix[i, j]), 2)); /* элемент возводим 
                    во вторую степень. (Convert.ToInt32)- делаем конвертацию в целое число, иначе 
